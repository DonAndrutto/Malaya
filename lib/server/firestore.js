...
// ─────────────────────────────────────────────────────────────────────────────
// THE cache policy constant. Every server-side Firestore read in this project
// (fetchDoc / fetchCollection / the unstable_cache wrappers in this file and
// lib/server/explore.js) revalidates on this interval, and — because Next
// takes the *lowest* revalidate seen while rendering a route — it is also what
// sets the ISR interval of every storefront route.
//
// 24 hours, not minutes, and deliberately so: freshness is not the TTL's job.
// Admin writes purge these caches on demand the moment Firestore ACKs them
// (lib/revalidate-ping.js → app/api/revalidate → revalidateTag/revalidatePath),
// so a published change is live on the very next request. The TTL exists only
// as the safety net for the case where that ping never lands (admin offline,
// tab closed mid-flight, endpoint 5xx): at worst the site self-heals within a
// day instead of staying stale forever.
//
// The previous value was 300s. ISR is request-driven, not cron-driven: the
// TTL does not schedule anything, it only decides when a cached entry becomes
// *eligible* for regeneration — the regeneration (and its ISR write) happens
// when a request arrives for an entry that has gone stale. At 300s a route
// became eligible up to 288 times a day, so a route walked regularly by
// crawlers could be regenerated on that cadence, writing a fresh ISR entry
// each time for content that changes a few times a week. With traffic far too
// low to explain the observed usage, that repeated crawler-triggered
// regeneration is the leading explanation for the Vercel Hobby ISR-write
// allowance going over quota. See CACHING.md.
//
// Route segment configs (`export const revalidate = …`) must be statically
// analyzable literals, so they cannot import this constant; the literals are
// pinned to it by lib/server/cache-policy.test.js instead.
// ─────────────────────────────────────────────────────────────────────────────
...
// Published blog posts → { [slug]: post }. Two layers, doing different jobs:
//   unstable_cache — the cross-request ISR/data cache (POST requests bypass
//                    the fetch cache, so the wrapper is what provides the TTL
//                    and the BLOG_CACHE_TAG purge hook).
//   react cache    — per-request memoisation. The layout, the page and
//                    generateMetadata all ask for this within one render; the
//                    unstable_cache layer would answer each from the cache
//                    store and re-deserialise the payload every time. One
//                    lookup per request instead of three or four.
export const fetchPublishedBlogPosts = cache(unstable_cache(
  async () => {
