...
//      allowance over quota. Intentional departures from the general policy
//      live in REVALIDATE_EXCEPTIONS below, where they have to be spelled out
//      and justified; anything else must equal the constant.
...
// Routes that deliberately do NOT follow the 24h safety-net policy, keyed by
// repo-relative path. Every entry is a decision with a reason recorded in the
// file itself and in CACHING.md — not tolerated drift.
//
//   app/sitemap.js — 1h. Next 14's filesystem cache handler ignores cache
//     tags for entries of kind ROUTE, so on-demand purges of a metadata route
//     cannot be relied on when self-hosting and are unverified on Vercel. An
//     hourly floor keeps newly published URLs discoverable for ~720 writes a
//     month, under 1% of the allowance.
const REVALIDATE_EXCEPTIONS = {
  'app/sitemap.js': 3600,
};
...
describe('cache policy', () => {
  ...
  test('every route segment `revalidate` matches the policy for that route', () => {
    ...
    found.forEach(([file, value]) => {
      const expected = String(REVALIDATE_EXCEPTIONS[file] ?? REVALIDATE_SECONDS);
      expect([file, value]).toEqual([file, expected]);
    });
  });

  test('the declared exceptions still exist and are still exceptional', () => {
    // An exception that no longer applies is worse than no exception: it
    // silently licenses a value nobody re-argued. Fail if one names a file
    // that has gone, or if it has drifted back to the general policy (at
    // which point the entry should simply be deleted).
    Object.entries(REVALIDATE_EXCEPTIONS).forEach(([file, seconds]) => {
      expect(fs.existsSync(path.join(ROOT, file))).toBe(true);
      expect(seconds).not.toBe(REVALIDATE_SECONDS);
    });
  });
