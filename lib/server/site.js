...
import { fetchCollection, fetchDoc, fetchPublishedBlogPosts } from './firestore';
...
export async function getServerLayoutData() {
  const [overrides, settings, savedContent, blogPosts] = await Promise.all([
    fetchCollection('catalogueOverrides'),
    fetchDoc('siteSettings/images'),
    fetchDoc('siteSettings/content'),
...
export async function getServerSiteData() {
  const overrides = await fetchCollection('catalogueOverrides');
...
export async function getServerContent() {
  const saved = await fetchDoc('siteSettings/content');
