...
// A brand-new item created in the admin Stock-ledger desk. It has no physical
// ledger row behind it — the override doc IS the whole record (marked `_custom`).
// We synthesise a ledger row from the saved fields and run it through the same
// resolver so published/sale-price/images/story behave exactly like a real line.
export function resolveCustomStorefront(id, o = {}) {
  const sku = o.salesCode || id;
  const row = {
    sku,
    code: o.productionCode || String(sku).split('-')[0],
    name: o.name || 'Untitled item',
    category: o.category || 'Accessories',
    material: o.material || '',
    qty: o.qty == null || o.qty === '' ? null : Number(o.qty),
    cost: o.unitCost == null || o.unitCost === '' ? null : Number(o.unitCost),
    retail: o.retail == null || o.retail === '' ? null : Number(o.retail),
    productIds: [],
  };
  // Route/identity stays the override key so the storefront URL is stable even
  // if the sales code is later edited.
  return { ...resolveLedgerStorefront(row, o), id, ledger: true, custom: true };
}
