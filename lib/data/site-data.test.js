...
// Homepage hero slides (resolveHeroSlides): collection slides when configured,
// legacy plain slideshow otherwise.
describe('resolveHeroSlides', () => {
  const cfg = {
    heroCollections: {
      'Heart Syllables': { img: 'https://x/hs.jpg', order: 2, enabled: true },
      'Ritual Objects': { img: 'https://x/ro.jpg', order: 1, enabled: true, cta: 'Discover Ritual Objects' },
      'Healing Stones': { img: 'https://x/no.jpg', enabled: false }, // disabled → hidden
      'Mystical Beings': { enabled: true }, // no image → hidden
    },
    heroSlides: ['https://x/legacy.jpg'],
  };

  test('enabled collections with an image become ordered slides with View CTAs', () => {
    const slides = resolveHeroSlides(cfg);
    expect(slides.map((s) => s.collection)).toEqual(['Ritual Objects', 'Heart Syllables']);
    expect(slides[0].cta).toBe('Discover Ritual Objects'); // admin override
    expect(slides[1].cta).toBe('View Heart Syllables'); // default CTA format
    expect(slides[1].href).toBe(`#coll-${encodeURIComponent('Heart Syllables')}`);
  });

  test('falls back to the legacy plain slideshow when no collection slide is live', () => {
    ...
    // No collection binding: the component falls back to content.hero.cta.
    expect(slides[0].collection).toBeNull();
