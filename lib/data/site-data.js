...
// ── Builder ──────────────────────────────────────────────────────────────────
// Resolve the studio catalogue against the current admin overrides, drop hidden
// items, flag Tashi pieces, then append the live-site extras.
export function buildSiteData(overrides = {}) {
  ...
  // Live-site extras carry no price-sheet entry, but they can still have their
  // image replaced (admin upload or the seed script), stored as an `img`
  // override under their id — apply it here.
  const extras = SITE_EXTRAS.map((p) => {
    const o = overrides[p.id];
    return o && o.img ? { ...p, img: o.img } : p;
  });

  const SITE_PRODUCTS = [...catalogue, ...extras];
