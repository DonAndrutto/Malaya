...
// Preferred source: the admin's per-collection hero config
// (settings.heroCollections — { [collection]: { img, order, cta, enabled } },
// edited in the admin Site Images tab). Each enabled collection with an image
// becomes one slide carrying that collection's hero image and a
// "View <Collection>" CTA that deep-links to the catalogue filtered to the
// collection (/#coll-<name>, handled by CatalogueScroll). Falls back to the
// legacy plain slideshow (settings.heroSlides) so existing sites render
// unchanged until the studio configures collection slides. Shared by the
// HomePage client component and the server layout's LCP preload, so both
// always agree on the first slide.
export function resolveHeroSlides(settings) {
  const cfg = (settings && settings.heroCollections) || {};
  const slides = Object.keys(cfg)
    .map((name) => ({ name, ...(cfg[name] || {}) }))
    ...
    .map((c) => ({
      src: c.img,
      collection: c.name,
      cta: (c.cta || '').trim() || `View ${c.name}`,
      href: `#coll-${encodeURIComponent(c.name)}`,
    }));
  ...
  return legacy.map((src) => ({ src, collection: null, cta: null, href: '#catalogue' }));
}
