...
// "In Stock" storefront filter predicate: pieces physically on the shelf.
// 'In stock' subsumes the retired 'Low stock' status (stock-data.js merged
// them; legacy overrides may still carry the old string) — both count as on
// hand. Everything else (Sold out, Made to order, Archived) is excluded.
export function isInStock(p) {
  return /^(in|low)[\s-]*stock$/i.test(String((p && p.stock) || '').trim());
}
...
// ── Catalogue ordering ───────────────────────────────────────────────────────
// The admin already tracks each piece's inventory (Units on hand, in the
// Inventory desk) and its Sold-out/New status — nothing new to store, just a
// display order derived from data that already exists:
//   1. Higher available stock first (an untracked/made-to-order piece — qty
//      unknown — ranks below any counted quantity, but is still "in stock").
//   2. Every Sold-out piece sinks below every in-stock piece, regardless of (1).
//   3. Among Sold-out pieces, ones marked New surface first.
//   4. Anything still tied keeps its prior relative order — Array#sort is a
//      stable sort (spec-guaranteed since ES2019), so simply not reordering
//      equal-ranked pieces is enough.
export function compareCatalogueOrder(a, b) {
  const aOut = a.stock === 'Sold out';
  const bOut = b.stock === 'Sold out';
  if (aOut !== bOut) return aOut ? 1 : -1;
  if (!aOut) {
    const aQty = typeof a.qty === 'number' ? a.qty : -1;
    const bQty = typeof b.qty === 'number' ? b.qty : -1;
    return bQty - aQty;
  }
  const aNew = (a.specials || []).includes('new') || a.tag === 'new';
  const bNew = (b.specials || []).includes('new') || b.tag === 'new';
...
export function buildSiteData(overrides = {}) {
  ...
  // master listing carries it. The catalogue-wide sort runs once here (see
  // compareCatalogueOrder) rather than in every filtered view that reads
  // SITE_PRODUCTS, so category/metal/symbol/search filtering downstream is a
  // plain Array#filter that preserves this order for free.
  const SITE_PRODUCTS = [...ledgerProducts, ...catalogue, ...extras, ...customs]
    .filter((p) => !isMerged(p.id))
  ...
  return { SITE_PRODUCTS, SITE_BY_ID, TASHI_PRODUCTS, ALIASES };
}
...
// ── Catalogue sections ────────────────────────────────────────────────────────
// The combined home catalogue is one long scroll, grouped by category in this
// fixed order (some categories merge into a single section). Shared by the
// home page (section anchors), the admin's hero-slide config and
// resolveHeroSlides, so a hero CTA always lands on a real /#cat-<key> anchor.
...
// Preferred source: the admin's per-category hero config
// (settings.heroCategories — { [sectionKey]: { img, order, cta, enabled } },
// edited in the admin Site Images tab). Each enabled catalogue section with an
// image becomes one slide carrying that category's hero image and a
// "View All <Category>" CTA anchored to the category's section of the home
// catalogue scroll (/#cat-<key>). The hero promotes product categories —
// Rings, Pendants, Bracelets… — never collections. Falls back to the legacy
// plain slideshow (settings.heroSlides) so existing sites render unchanged
// until the studio configures category slides. Shared by the HomePage client
// component and the server layout's LCP preload, so both always agree on the
// first slide.
export function resolveHeroSlides(settings) {
  const cfg = (settings && settings.heroCategories) || {};
  const slides = CATALOGUE_SECTIONS
    .map((s, i) => ({ s, c: cfg[s.key] || {}, i }))
    .filter(({ c }) => c.enabled && c.img)
    .sort((a, b) => ((a.c.order ?? 1e9) - (b.c.order ?? 1e9)) || (a.i - b.i))
    .map(({ s, c }) => ({
      src: c.img,
      category: s.key,
      cta: (c.cta || '').trim() || `View All ${s.label}`,
      href: `#cat-${s.key}`,
    }));
  if (slides.length) return slides;
  const legacy = (settings && Array.isArray(settings.heroSlides) && settings.heroSlides.length)
    ? settings.heroSlides
    : HOME_HERO;
  // cta null → the component falls back to the admin-editable content.hero.cta.
  return legacy.map((src) => ({ src, category: null, cta: null, href: '#catalogue' }));
}
...
export const CONTENT_DEFAULTS = {
  nav: { home: 'Home', catalogue: 'Catalogue', explore: 'Explore', tashi: 'Tashi Mannox', blog: 'Blog', contact: 'Contact', about: 'About' },
