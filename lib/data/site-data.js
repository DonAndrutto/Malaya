...
import { STOCK_ROWS, resolveLedgerStorefront, resolveCustomStorefront } from './ledger';
...
export function buildSiteData(overrides = {}) {
  // Items the admin has marked deleted (reserved `_deleted` flag) never appear.
  ...
  const ledgerProducts = STOCK_ROWS
    ...
    .filter((p) => p.published && !HIDDEN_STOCK.includes(p.stock) && !isDeleted(p.id))
    .map((p) => ({ ...p, tashi: false }));

  // 2. Brand-new items created in the admin (reserved `_custom` override docs),
  //    published the same way a real ledger line is.
  const customProducts = Object.keys(overrides)
    .filter((id) => overrides[id] && overrides[id]._custom && !overrides[id]._deleted)
    .map((id) => resolveCustomStorefront(id, overrides[id]))
  ...
  const catalogue = resolveCatalogue(PRODUCTS, overrides)
    .filter((p) => !HIDDEN_STOCK.includes(p.stock) && !superseded.has(p.id) && !isDeleted(p.id))
  ...
  const SITE_PRODUCTS = [...ledgerProducts, ...customProducts, ...catalogue, ...extras];
