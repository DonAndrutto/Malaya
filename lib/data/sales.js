...
// Filters: free-text query, status key, clientId (or exact customer name for
// unlinked orders), currency, and an inclusive created-at date range
...
export function filterOrders(orders, { q = '', status = '', customer = '', currency = '', from = '', to = '' } = {}) {
  ...
  return (orders || []).filter((o) => {
    ...
    if (customer) {
      const c = o.customer || {};
      if (c.clientId !== customer && (c.name || '').trim() !== customer) return false;
    }
...
//
//   • a ring line with a chosen size and per-size tracking decrements that size,
//     recomputing the on-hand total and status exactly like the Inventory desk
//     (total > 0 → 'In stock', 0 → 'Made to order'; Archived stays put)
...
// override patches; the caller applies them and flags the order so the
// deduction can only ever run once per order.
...
export function inventoryDeductionPatches(order, byId) {
  ...
  ((order && order.items) || []).forEach((it) => {
    ...
    if (sizeKey && p.sizes && p.sizes[sizeKey] != null) {
      const sizes = { ...p.sizes, [sizeKey]: Math.max(0, (Number(p.sizes[sizeKey]) || 0) - qty) };
      const total = Object.keys(sizes).reduce((s, k) => s + (Number(sizes[k]) || 0), 0);
      const patch = { sizes, qty: total };
      if (p.stock !== 'Archived') patch.stock = total > 0 ? 'In stock' : 'Made to order';
      patches.push({ id: p.id, label: `${p.name} · size ${sizeKey} −${qty}`, patch });
    } else if (p.qty != null) {
      patches.push({ id: p.id, label: `${p.name} −${qty}`, patch: { qty: Math.max(0, p.qty - qty) } });
    }
  });
  return patches;
}
