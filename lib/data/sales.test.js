import {
  ...
  planInventoryDeduction, inventoryRestockPatches,
...
describe('filters & sorts', () => {
  ...
  test('namespaced customer values keep a client and a same-name walk-in apart', () => {
    const withWalkIn = [
      ...orders,
      order({ id: 'd', number: 'MJ-2026-0004', customer: { clientId: null, name: 'Ana' } }),
    ];
    expect(filterOrders(withWalkIn, { customer: 'client:k1' }).map((o) => o.id)).toEqual(['a', 'c']);
    expect(filterOrders(withWalkIn, { customer: 'name:Ana' }).map((o) => o.id)).toEqual(['d']);
  });
...
describe('duplicateOrder', () => {
  test('copies customer, currency, items and production notes; resets lifecycle', () => {
    const src = order({
      ...
      inventoryDeductions: [{ id: 'r1', size: '50', qty: 1 }],
    });
    ...
    expect(dup.inventoryDeductions).toEqual([]); // a duplicate deducts as a fresh sale
...
describe('inventory deduction', () => {
  ...
  test('ring line with a chosen size decrements that size and recomputes the total', () => {
    ...
    // Units remain, so the studio's availability wording is left alone.
    expect(patches[0].patch).toEqual({ sizes: { 50: 0, 52: 3 }, qty: 3 });
  });

  test('two sizes of the same ring on one order both come off the ladder', () => {
    const o = order({
      items: [
        item({ productId: 'r1', ringSize: 50, qty: 1 }),
        item({ productId: 'r1', ringSize: 52, qty: 1 }),
      ],
    });
    const { patches, movements } = planInventoryDeduction(o, byId);
    expect(patches).toHaveLength(1); // one compounded patch, not last-line-wins
    expect(patches[0].patch).toEqual({ sizes: { 50: 0, 52: 2 }, qty: 2 });
  ...
  test('two plain lines of the same product compound instead of last-write-wins', () => {
    const o = order({ items: [item({ productId: 'p1', qty: 1 }), item({ productId: 'p1', qty: 1 })] });
    const { patches, movements } = planInventoryDeduction(o, byId);
    expect(patches).toEqual([{ id: 'p1', label: 'Vajra Pendant −1 · Vajra Pendant −1', patch: { qty: 0 } }]);
    expect(movements).toEqual([{ id: 'p1', size: null, qty: 1 }, { id: 'p1', size: null, qty: 1 }]);
  });

  test('a size outside the tracked ladder deducts nothing and never touches pooled qty', () => {
    const o = order({ items: [item({ productId: 'r1', ringSize: 60, qty: 1 })] });
    expect(planInventoryDeduction(o, byId)).toEqual({ patches: [], movements: [] });
  });

  test('a sized item without a chosen size is skipped, not pooled', () => {
    const o = order({ items: [item({ productId: 'r1', ringSize: null, qty: 1 })] });
    expect(planInventoryDeduction(o, byId)).toEqual({ patches: [], movements: [] });
  });

  test('deduction never resurrects a studio-set Made to order back to In stock', () => {
    const made = { r1: { ...byId.r1, stock: 'Made to order' } };
    const o = order({ items: [item({ productId: 'r1', ringSize: 50, qty: 1 })] });
    expect(planInventoryDeduction(o, made).patches[0].patch.stock).toBeUndefined();
  });
  ...
  test('plain line decrements qty (floored at 0) and leaves status alone', () => {
    ...
    const { patches, movements } = planInventoryDeduction(o, byId);
    // Only 2 were on hand: the label and the recorded movement say what
    // actually came off, so a cancellation can't restore more than was taken.
    expect(patches).toEqual([{ id: 'p1', label: 'Vajra Pendant −2', patch: { qty: 0 } }]);
    expect(movements).toEqual([{ id: 'p1', size: null, qty: 2 }]);
  });
...
describe('inventory restock', () => {
  // Product state as it stands AFTER the deduction ran.
  const after = {
    r1: { id: 'r1', name: 'Dorje Ring', category: 'Rings', stock: 'Made to order', qty: 2, sizes: { 50: 0, 52: 2 } },
    p1: { id: 'p1', name: 'Vajra Pendant', category: 'Pendants', stock: 'In stock', qty: 0, sizes: null },
  };
  const deductions = [
    { id: 'r1', size: '50', qty: 1 },
    { id: 'r1', size: '52', qty: 1 },
    { id: 'p1', size: null, qty: 2 },
  ];

  test('returns exactly the recorded movements to their size / pooled counters', () => {
    const o = order({ inventoryDeducted: true, inventoryDeductions: deductions });
    const patches = inventoryRestockPatches(o, after);
    expect(patches).toEqual([
      { id: 'r1', label: 'Dorje Ring · size 50 +1 · Dorje Ring · size 52 +1', patch: { sizes: { 50: 1, 52: 3 }, qty: 4 } },
      { id: 'p1', label: 'Vajra Pendant +2', patch: { qty: 2 } },
    ]);
    // Availability wording is never upgraded automatically — no stock field.
    expect(patches.every((p) => !('stock' in p.patch))).toBe(true);
  });

  test('orders without recorded movements restock nothing (legacy deductions)', () => {
    expect(inventoryRestockPatches(order({ inventoryDeducted: true }), after)).toEqual([]);
  });

  test('movements for products that no longer resolve are skipped', () => {
    const o = order({ inventoryDeducted: true, inventoryDeductions: [{ id: 'ghost', size: null, qty: 1 }] });
    expect(inventoryRestockPatches(o, after)).toEqual([]);
  });

  test('sized movement falls back to pooled qty when the ladder was removed', () => {
    const noLadder = { r1: { id: 'r1', name: 'Dorje Ring', stock: 'Made to order', qty: 2, sizes: null } };
    const o = order({ inventoryDeducted: true, inventoryDeductions: [{ id: 'r1', size: '50', qty: 1 }] });
    expect(inventoryRestockPatches(o, noLadder)).toEqual([
      { id: 'r1', label: 'Dorje Ring +1', patch: { qty: 3 } },
    ]);
  });
});
