...
// ── Shopper-facing metal family (storefront Silver / Gold filter) ────────────
// Simpler than METAL_GROUPS: every karat/colour of solid gold *and platinum*
// reads as "gold"; sterling silver and the gold-plated pieces (silver core) read
// as "silver". Used by the catalogue material filter.
export const MATERIAL_FAMILIES = {
  gold: ['14k Yellow Gold', '18k Yellow Gold', '14k White Gold', '18k White Gold', '14k Rose Gold', '18k Rose Gold', 'Platinum'],
  silver: ['Silver 925', 'Yellow Gold Plated', 'Rose Gold Plated'],
};

export function materialFamilyOf(material) {
  const m = normalizeMaterial(material);
  if (MATERIAL_FAMILIES.gold.includes(m)) return 'gold';
  if (MATERIAL_FAMILIES.silver.includes(m)) return 'silver';
  return null;
}
