...
// In-tab listeners so demo-mode writes re-emit immediately (the browser's
// `storage` event only fires in *other* tabs; with Firebase, onSnapshot's
// latency compensation plays this role instead).
let salesListeners = [];
...
export function subscribeSales(cb) {
  ...
  const f = (data) => cb(normalize(data));
  salesListeners.push(f);
  window.addEventListener('storage', onStorage);
  return () => {
    salesListeners = salesListeners.filter((x) => x !== f);
