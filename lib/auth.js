...
// The exact two-leg check firestore.rules isAdmin() performs, evaluated from
// the client's own vantage point: the live ID token (force-refreshed, so a
// grant made moments ago is picked up without re-login) and the account's own
// admins/{uid} allowlist doc (the rules allow reading exactly one's own
// entry). Being signed in is NOT being an admin — with Email/Password
// enabled anyone can hold a valid session while every admin-gated read and
// write is rejected with permission-denied. Returns null when signed out,
// otherwise { uid, email, claim, allowlisted, isAdmin }.
export async function checkAdminRights() {
  const { auth, db } = getFirebase();
  const u = auth && auth.currentUser;
  if (!u || !db) return null;
  let claim = false;
  try { claim = (await u.getIdTokenResult(true)).claims.admin === true; } catch {}
  let allowlisted = false;
  try { allowlisted = (await getDoc(doc(db, 'admins', u.uid))).exists(); } catch {}
  return { uid: u.uid, email: u.email || '', claim, allowlisted, isAdmin: claim || allowlisted };
}
