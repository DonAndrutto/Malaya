  ...
  try {
    ...
    ids.forEach((id) => {
      ...
      if (pub && Object.keys(pub).length) {
        setDoc(doc(db, COLLECTION, id), { ...pub, _updated: Date.now() }).catch(logWriteError(id));
      } else {
        deleteDoc(doc(db, COLLECTION, id)).catch(logWriteError(id));
      }
      if (priv) {
        setDoc(doc(db, PRIVATE_COLLECTION, id), { ...priv, _updated: Date.now() }).catch(logWriteError(id));
      } else if (prevPriv) {
        deleteDoc(doc(db, PRIVATE_COLLECTION, id)).catch(logWriteError(id));
      }
    });
  } catch {}
}
...
//
// Options:
//   skipCache      — don't emit the localStorage cache up front (used when the
//                    caller already has fresher server-rendered data).
