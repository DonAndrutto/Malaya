...
// "saved": broadcast it so the admin UI can warn (ExploreAdmin listens).
export const EXPLORE_SAVE_ERROR_EVENT = 'malaya:explore-save-error';
function notifySaveError(e) {
  try {
    window.dispatchEvent(new CustomEvent(EXPLORE_SAVE_ERROR_EVENT, { detail: String((e && e.message) || e) }));
  } catch {}
}
...
function saveExploreDoc(key, collectionName, slug, data) {
  ...
  if (!db) return;
  try {
    if (data) setDoc(doc(db, collectionName, slug), { ...data, slug, _updated: Date.now() }).catch(notifySaveError);
    else deleteDoc(doc(db, collectionName, slug)).catch(notifySaveError);
  } catch {}
}
...
export function saveTopic(slug, topic) {
  ...
  saveExploreDoc(EXPLORE_TOPICS_KEY, TOPICS, slug, topic);
}
export function deleteTopic(slug) {
  checkpointTopic(slug);
  saveExploreDoc(EXPLORE_TOPICS_KEY, TOPICS, slug, null);
}
